...
def get_low_confidence_regions(
    ...
    """
    Identify spatial regions with low confidence for targeted drone coverage.
    
    Args:
        world_points: 3D world coordinates
        confidence: Confidence scores
        conf_threshold: Percentile threshold as decimal (0.0-1.0)
        cluster_radius: Radius for clustering low-confidence points
        
    Returns:
        Dictionary containing low-confidence regions and suggested coverage areas
    """
    # Convert tensors to numpy if needed
    if isinstance(world_points, torch.Tensor):
        world_points = world_points.detach().cpu().numpy()
    ...
    low_conf_mask = (confidence < actual_threshold) & np.isfinite(confidence)
    low_conf_points = world_points[low_conf_mask]
    
    if len(low_conf_points) == 0:
        return {
            "low_confidence_points": np.array([]),
            "cluster_centers": np.array([]),
            "coverage_suggestions": np.array([])
        }
    ...
    try:
        ...
        cluster_labels = clustering.fit_predict(low_conf_points)
        ...
        for label in unique_labels:
            cluster_points = low_conf_points[cluster_labels == label]
        ...
        print("Warning: scikit-learn not available, using simple clustering")
        # Fallback to simple grid-based clustering
        cluster_centers = simple_grid_clustering(low_conf_points, cluster_radius)
    
    return {
        "low_confidence_points": low_conf_points,
        "cluster_centers": cluster_centers,
        "coverage_suggestions": cluster_centers  # For now, same as cluster centers
    }


def simple_grid_clustering(points: np.ndarray, grid_size: float) -> np.ndarray:
    """
    ...
    # Find unique grid cells with points
    unique_cells = np.unique(grid_indices, axis=0)
    
    # Calculate center of each occupied cell
    cluster_centers = []
