...
def predictions_to_glb(
    ...
    Args:
        ...
        mask_dynamic (bool): Apply semantic segmentation to filter dynamic objects (default: False)
...
def extract_point_cloud_data(
    ...
    """
    Extract point cloud data (vertices and colors) using the same logic as predictions_to_glb.
    
    Returns:
        tuple: (vertices_3d, colors_rgb) where:
            - vertices_3d: numpy array of shape (N, 3) with 3D coordinates
            - colors_rgb: numpy array of shape (N, 3) with RGB colors (0-255)
    """
    if not isinstance(predictions, dict):
        raise ValueError("predictions must be a dictionary")
    if conf_thres is None:
...
def save_point_cloud_ply(vertices, colors, filename):
    """
    Save point cloud data to PLY format with colors.
    
    Args:
        vertices: numpy array of shape (N, 3) with 3D coordinates
        colors: numpy array of shape (N, 3) with RGB colors (0-255)
        filename: output PLY file path
    """
    print(f"[Visual Utils] Saving PLY file to: {filename}")
...
"""
    ...
    print(f"[Visual Utils] PLY file saved: {filename}")
...
def export_point_cloud_to_ply(
    ...
    """
    Generate point cloud data and save to PLY format using the same processing logic as GLB export.
    ...
    Args:
        ...
        mask_black_bg: Whether to mask black background
        mask_white_bg: Whether to mask white background
        mask_sky: Whether to apply sky segmentation mask
        mask_dynamic: Whether to apply semantic filtering for dynamic objects
        target_dir: Target directory for intermediate files
        prediction_mode: Prediction mode selector
        
    Returns:
        tuple: (vertices_3d, colors_rgb) point cloud data
    """
    # Extract point cloud data using the same logic as GLB export
    vertices_3d, colors_rgb = extract_point_cloud_data(
        predictions,
    ...
    # Return point cloud data for caller to decide whether to save
    return vertices_3d, colors_rgb
