...
def apply_sky_mask(conf, target_dir):
    """Zero confidence on sky pixels for each frame. No-op when ``target_dir`` is None."""
    ...
    if not os.path.exists(constants.SKYSEG_ONNX_PATH):
        ...
        _download(constants.SKYSEG_ONNX_URL, constants.SKYSEG_ONNX_PATH)
