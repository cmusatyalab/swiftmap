...
Compatible with the vggt_localization test client protocol but adapted for mapping workflow.

Features:
- Sends images from directory to TCP server
- Receives keyframe selection status responses
- Displays real-time statistics
- Compatible with existing vggt_localization test client protocol
...
class MappingTestClient:
    ...
    def connect(self) -> bool:
        ...
        """
        try:
            print(f"📡 Connecting to VGGT Mapping server at {self.host}:{self.port}...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test client for VGGT Mapping TCP server",
...
Examples:
  ...
  %(prog)s --test-connection                           # Test connection only
    ...
    print("="*60)
    print("🚁 VGGT MAPPING TEST CLIENT")
    print("="*60)
